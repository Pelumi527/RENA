...
const ClaimModal = () => {
  ...
  const lastRenegadesData = useAppSelector(
    (state) => state.renegadesState.lastRenegadesData,
  );
  const isLRDLoading = useAppSelector(
    (state) => state.renegadesState.isLRDLoading,
  );
  ...
  return (
    ...
    >
      <div
        style={{
          backgroundImage: `url("/renegades/bg-model.png")`,
        ...
        <div className="flex flex-col w-full">
          <div className="flex justify-between items-center">
            <p className="text-[26px] font-semibold text-[#FFF] leading-[130%]">
              {proceed == 0
                ? "Claim a Renegade"
                : `You’ve got ${lastRenegadesData?.token_count && lastRenegadesData?.token_count > 1 ? lastRenegadesData?.token_count : "a"} new Renegade${lastRenegadesData?.token_count && lastRenegadesData?.token_count > 1 ? "s" : ""}!`}
            </p>
          ...
          <div className={`flex flex-col items-center justify-between mt-8`}>
            <div className="flex flex-col items-center relative">
              {proceed > 0 || isLRDLoading ? (
                <>
                  <LoadingImageClaim
                    url={lastRenegadesData?.token_uri}
                    className="w-[194px] h-[194px] rounded-[8px] mt-2"
                  />
                  ...
                  >
                    <div
                      className={`leading-[130%] text-[18px] font-bold flex items-center justify-center ${currentRank && levelClass(currentRank)[0]}`}
                    >
                      <Icon
                        icon={"ph:medal-fill"}
                        fontSize={20}
                  ...
                  {lastRenegadesData?.token_count &&
                    lastRenegadesData?.token_count > 1 && (
                      <div className="absolute -right-[60px] -top-[16px] bg-primary border-2 border-[#FFF] w-[123px] h-[46px] rounded-[8px] flex items-center justify-center text-[22px] font-semibold">
                        + {lastRenegadesData?.token_count - 1} more
                      </div>
                    )}
                </>
              ) : (
                <img
                  ...
                  className="w-[194px] h-[194px] rounded-[8px]"
                />
              )}
            </div>
              ...
              <>
                <div className="flex h-12 justify-center gap-4 items-center mt-9">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      renaBalance < Number(e.target.value)
                        ? setCount(renaBalance)
            ...
            <div className="flex sm:flex-row flex-col justify-center gap-4 sm:gap-6 mt-8 mb-2 w-full">
              {Math.floor(renaBalance) > 0 && (
                <PrimaryButton
                  onClick={onClaim}
                  className="block sm:hidden !font-bold text-[18px] w-full sm:w-[203px] h-12"
                >
                  Claim Renegade{count > 1 && "s"}
                </PrimaryButton>
              )}
              {Math.floor(renaBalance) > 0 ? (
                <SecondaryButton
              ...
              {Math.floor(renaBalance) > 0 && (
                <PrimaryButton
                  onClick={onClaim}
                  className="hidden sm:block !font-bold text-[18px] w-full sm:w-[203px] !h-12"
                >
                  Claim Renegade{count > 1 && "s"}
                </PrimaryButton>
              )}
