...
const FifthSection = () => {
  ...
  const faqs = [
    ...
    },
    {
      ...
      answer:
        "Liquid NFT is built by harnessing the power of Aptos Move Objects. Instead of coding the relation between the fungible token and the NFT into the smart contract on ERC-404, Liquid NFT achieves semi-fungibility on the language level. Unique to the Aptos Move language, Aptos Objects allow for a composable relation between the fungible token and the non-fungible token. Each token (fungible or non-fungible) is an object that can be embedded inside one another. So when user claims an NFT with fungible token or liquifies an NFT on Liquid NFT, the fungible token is embedded and taken out of the NFT respectively. This is ONLY POSSIBLE ON APTOS.",
    },
    {
      question: "What is Renegades?",
      answer:
        "Renegades is the 1st Liquid NFT collection on Aptos leveraging the Liquid NFT standard and supported by the Aptos team and community. It is a collection of 5,000 Renegades coupled with the $RENA token, representing a 1:1 relationship for claiming and liquifying.",
    },
    {
      question: "Anything else?",
      answer:
        "We are bullish on Liquid NFTs, Composable NFTs, and all other native innovations only possible on Aptos.",
    },
  ];
        ...
        <div className="w-[95%] lg:w-[1100px] flex flex-col mt-[56px] border-y border-[#666]">
          {faqs.map((faq, index) => (
            <div key={index} className="px-6 border-b border-[#666]">
              ...
              >
                <p className="text-[17px] sm:text-[18px] pb-6 font-semibold text-gray-light">
                  {faq.answer}
                </p>
              </div>
            </div>
